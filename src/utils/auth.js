...
async function sayAuth(sendID, userID, name, auth, type, bot) {
  await bot.say(sendID, `您当前无${auth}权限。`, type, userID);
}
...
async function setAuth(auth, target, isOn) {
  ...
  if (undefined === data) {
    ...
    await db.update("authority", "user", { userID: target }, { ...data, [auth]: isOn });
  }
}
...
export { sayAuth, setAuth, hasAuth };
