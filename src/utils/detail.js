...
import { get, isInside, push, update } from "./database";
import { getBase, getDetail, getCharacters, getAbyDetail } from "./api";
var module = {
  exports: {}
};
...
const getCookie = async (uid, use_cookie) => {
  return new Promise(async (resolve, reject) => {
    ...
    let {
      date,
      cookie
    } = await get("cookies", "uid", {
      uid
    });
    let today = new Date().toLocaleDateString();

    if (date && cookie && date == today) {} else {
...
const abyPromise = async (uid, server, schedule_type) => {
  const {
    ...
    message,
    data
  } = await getAbyDetail(uid, schedule_type, server, await getCookie(uid, true));
  return new Promise(async (resolve, reject) => {
    if (retcode !== 0) {
...
const basePromise = async (mhyID, userID) => {
  const {
    ...
    message,
    data
  } = await getBase(mhyID, await getCookie("MHY" + mhyID, false));
  return new Promise(async (resolve, reject) => {
    if (retcode !== 0) {
      reject("米游社接口报错: " + message);
      return;
...
const detailPromise = async (uid, server, userID) => {
  ...
  const {
    ...
    message,
    data
  } = await getDetail(uid, server, await getCookie(uid, true));
  return new Promise(async (resolve, reject) => {
    if (retcode !== 0) {
      await update("info", "user", {
        uid
...
const characterPromise = async (uid, server, character_ids) => {
  const {
    ...
    message,
    data
  } = await getCharacters(uid, server, character_ids, await getCookie(uid, true));
  return new Promise(async (resolve, reject) => {
    if (retcode !== 0) {
      reject("米游社接口报错: " + message);
      return;
