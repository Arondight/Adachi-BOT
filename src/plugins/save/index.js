...
async function Plugin(msg, bot) {
  ...
  if ("string" === typeof id) {
    await bot.say(msg.sid, id, msg.type, msg.uid);
    return;
  }

  if (hasEntrance(msg.text, "save", "save")) {
    if (!(await db.includes("map", "user", "userID", msg.uid))) {
      await db.push("map", "user", { userID: msg.uid, mhyID });
      await bot.say(msg.sid, `通行证绑定成功，${okMsg}`, msg.type, msg.uid);
      await setCacheTimeout(msg.uid, mhyID, bot);
    } else {
      await bot.say(msg.sid, existMsg, msg.type, msg.uid);
    }
  } else if (hasEntrance(msg.text, "save", "change")) {
    if (await db.includes("map", "user", "userID", msg.uid)) {
      await db.update("map", "user", { userID: msg.uid }, { mhyID });
      await bot.say(msg.sid, `通行证改绑成功，${okMsg}`, msg.type, msg.uid);
      await setCacheTimeout(msg.uid, mhyID, bot);
    } else {
      await bot.say(msg.sid, unexistMsg, msg.type, msg.uid);
    }
  }
}
