...
module.exports = async Message => {
  ...
  let sendID = type === "group" ? groupID : userID; // 【评分】命令和图片之间可以加任意个空格
  ...
  if (!(await hasAuth(userID, "rating")) || !(await hasAuth(sendID, "rating"))) {
  ...
  try {
    ...
    await bot.sendMessage(sendID, `[CQ:at,qq=${userID}] 您看上去没有发送圣遗物属性截图，${whisper}。`, type);
    return;
  }
  ...
  if (response.status == 200) {
    ...
    await bot.sendMessage(sendID, `[CQ:at,qq=${userID}] 没有正确接收到截图，请再试一次。`, type);
    return;
  }
  ...
  let body = JSON.stringify(form); // { "name": "勋绩之花", "pos": "生之花", "star": 5, "level": 20,
  ...
  response = await doPost("https://api.genshin.pub/api/v1/app/ocr", headers, body);

  if (200 != response.status) {
    await bot.sendMessage(sendID, `[CQ:at,qq=${userID}] AI 识别出错，${whisper}。`, type);
    return;
  }
  ...
  prop = ret;
  response = await doPost("https://api.genshin.pub/api/v1/relic/rate", headers, body); // { "total_score": 700.4420866489831, "total_percent": "77.83", "main_score": 0,
  ...
  if (response.status == 400) {
    if (hasKey(ret, "code") && ret["code"] == 50003) {
      await bot.sendMessage(sendID, `[CQ:at,qq=${userID}] 你上传了正确的截图，但是 AI 未能正确识别，请重新截图。`, type);
    } else {
      await bot.sendMessage(sendID, `[CQ:at,qq=${userID}] 圣遗物评分出错，${whisper}。`, type);
    }
...
==========`;
  ...
  await bot.sendMessage(sendID, `[CQ:at,qq=${userID}] 发生了一个未知错误，请再试一次。`, type);
};
