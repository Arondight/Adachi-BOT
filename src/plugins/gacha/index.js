import { getGachaResult } from "./gacha";
import { alias } from "../../utils/alias";
import { render } from "../../utils/render";
import { hasAuth, sendPrompt } from "../../utils/auth";
import { get, push, isInside, update } from "../../utils/database";
var module = {
  exports: {}
};
...
module.exports = async Message => {
  ...
  if (msg.includes("卡池")) {
    ...
    const table = await get("gacha", "data", {
      gacha_type: 302
    });
    const {
      path
    ...
    if (path["course"] === null) await bot.sendMessage(sendID, `[CQ:at,qq=${userID}] 当前未指定定轨武器。`, type);else await bot.sendMessage(sendID, `[CQ:at,qq=${userID}] 当前定轨 ` + table["upFiveStar"][path["course"]]["item_name"] + "\n命定值 " + path["fate"] + "。", type);
