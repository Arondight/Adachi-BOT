...
```patch
From b9c310b844adb89e2a8438cfb9583e32dd7bd00f Mon Sep 17 00:00:00 2001
...
插件的参数 `msg` 传递的 oicq 数据结构改变。原本的其中的 `message` 字段是一个可能包含[这些类型](https://github.com/takayama-lily/oicq/blob/54db7ee49f4281845770b8f17a8e01cbe6f97e5b/lib/message/elements.ts#L1-L166)的数组，但是为了统一 `message` 和 `raw_message` 字段，在 `../src/utils/load.js` 中剔除了 TextElem 之外的所有类型并只保留一个 TextElem 。除此之外，将 `raw_message` 和 `message` 中仅存的 TextElem 进行了统一。最后新增了字段 `atMe: boolean` 来表示这条消息是否 `@` 了机器人。

除此之外， `msg.raw_message` 和 `msg.message[0].text` 依次去除了 `@` 机器人的 CQ 码、命令前缀 `config.prefixes` 和行首空格，但是你应当总是使用 `msg.text` 而非这两个属性。
...
1. 字段 `key` 对应的 Array 中是否有包含 `predicate` 的对象，或字段 `key` 的对象中是否包含 `predicate` 。
2. 字段 `key` 路径 `path` 对应的 Array 中是否有包含 `predicate` 的对象，或字段 `key` 路径 `path` 的对象中是否包含 `predicate` 。
...
1. 字段 `key` 的对象。
2. 字段 `key` 路径 `path` 对应的对象。
3. 字段 `key` 对应的 Array 中包含 `predicate` 的对象。
4. 字段 `key` 路径 `path` 对应的 Array 中包含 `predicate` 的对象。
...
1. 字段 `key` 对应的 Array 中包含 `predicate` 的对象，或字段 `key` 对应的包含 `predicate` 的对象。
1. 字段 `key` 路径 `path` 对应的 Array 中包含 `predicate` 的对象，或字段 `key` 路径 `path` 对应的包含 `predicate` 的对象。
...
数据库 `name` 中字段 `key` 对应的 Array 中，将 `data` 合并到包含 `predicate` 的对象中。返回 boolean 。
...
> 2. 将新数据和旧数据合并形成一个新的对象（其中的数组全部覆盖为新数据）。
...
(function main() {
  ...
  if (db.has(name, "data", "p1[0].p2")) {
    ...
    console.log("data has path p1[0].p2,", `value "${p2}"`, "with type", typeof p2);
  }

  // 检查数据库中 user 字段下是否存在 id 值为 100 的对象
  if (db.includes(name, "user", { id: 100 })) {
    // 获取数据库中 user 字段下 id 值为 100 的对象
    console.log("Object with id is 100 in user:", db.get(name, "user", { id: 100 }));
  }

  // 检查数据库中 data 字段下路径 p1 的数组中是否存在 p2 的值为 "text" 的对象
  if (db.includes(name, "data", "p1", { p2: "text" })) {
    // 获取数据库中 data 字段下路径 p1 的数组中 p2 的值为 "text" 的对象
    console.log('Object with p2 is "text" in array of data.p1:', db.get(name, "data", "p1", { p2: "text" }));
  }

  // 检查数据库中 data 字段下路径 a.b 的对象中是否包含 c 为 "text" 的属性
  if (db.includes(name, "data", "a.b", { c: "text" })) {
    // 获取数据库中 data 字段下路径 p1 的数组中 p2 的值为 "text" 的对象
    console.log('Object with c is "text" data.a.b:', db.get(name, "data", "a.b", { c: "text" }));
  }

  console.log(`Update data for database ${name} ...`);
  // 删除数据库 user 字段的数组中 id 为 101 的对象
  db.remove(name, "user", { id: 101 });
  // 删除数据库 data 字段下路径 p1 的数组中 p2 为 "text" 的对象
  db.remove(name, "data", "p1", { p2: "text" });
  // 更新数据库 user 字段下 id 为 100 的对象
  db.update(name, "user", { id: 100 }, { id: 1000, name: "AA" });
  // 数据库 user 字段下插入一个 id 为 1001 的对象
  db.push(name, "user", { id: 1001, name: "BB" });
  // 数据库 data 字段下路径 p1 中插入一个 p22 为 "text2" 的对象
  ...
  console.log(`Update data for database ${name} ...`);
  // 删除数据库 data 字段下路径 p1[0] 的对象——如果其值的对象包含 p22 为 "text2" 的属性
  db.remove(name, "data", "p1[0]", { p22: "text2" });
  // 删除数据库 data 字段下路径 a.b 的对象——如果其值的对象包含 c 为 "text" 的属性
  db.remove(name, "data", "a.b", { c: "text" });
