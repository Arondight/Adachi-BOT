...
初始化名称为 `name` 的数据库。数据库已存在则加载其数据，数据库不存在则创建空数据库。

```js
db.init(name, defaultElement);
```

> `defaultElement` 默认为 `{ user: [] }` 。
...
得到数据库 `name` 的磁盘文件路径。避免以任何方式操作此文件。
...
判断数据库 `name` 中字段 `key` 对应的 Array 中的对象中，是否有值为 `value` 的索引 `index` 。
...
> | ---------------------- | ------------------------------------------------------ |
> | `undefined` （默认值） | `key` 对应的 Array                                     |
> | Object                 | `key` 对应的 Array 中的对象中，包含 `predicate` 的对象 |

##### 设置数据

数据库 `name` 中字段 `key` 对应的数据设置为 `data` 。
...
数据库 `name` 中字段 `key` 对应的 Array 中，将包含 `predicate` 的对象删除。
...
数据库 `name` 中字段 `key` 对应的 Array 中，插入一条数据 `data` 。
...
数据库 `name` 中字段 `key` 对应的 Array 中，将 `data` 合并到包含 `predicate` 的对象中。
...
(function main() {
  ...
  const name = "testDB";
  // 初始化字段，通常用于声明数据库的结构
  const defaultElement = { user: [], data: {} };
  let user = [];
  let data = {};
  ...
  user = db.get(name, "user") || [];
  data = db.get(name, "data") || {};
  console.log("user:", user);
  console.log("data:", data);

  // 检查数据库中 data 字段下是否存在 p1[0].p2 路径
  if (db.has(name, "data", "p1", 0, "p2")) {
    const p2 = data.p1[0].p2;
    console.log("data has path p1[0].p2,", `value "${p2}"`, "with type", typeof p2);
  }

  // 检查数据库中 user 字段下是否存在 id 为 100 的对象
  if (db.includes(name, "user", { id: 100 })) {
    // 获取数据库中 user 字段下 id 为 100 的对象
    console.log(db.get(name, "user", { id: 100 }) || {});
  }
  ...
  // 根据名字同步数据库缓存到磁盘
  // XXX 尽量避免使用 db.sync ，已有一个周期性的定时任务同步缓存，程序退出时也会做同步
