...
2. 字段 `key` 路径 `path` 对应的 Array 中是否有包含 `predicate` 的 object ，或字段 `key` 路径 `path` 的 object 中是否包含 `predicate` 。
...
1. 字段 `key` 的 object 。
2. 字段 `key` 路径 `path` 对应的 object 。
3. 字段 `key` 对应的 Array 中包含 `predicate` 的 object 。
4. 字段 `key` 路径 `path` 对应的 Array 中包含 `predicate` 的 object 。
...
1. 字段 `key` 路径 `path` 对应的 Array 中包含 `predicate` 的 object ，或字段 `key` 路径 `path` 对应的包含 `predicate` 的 object 。
...
数据库 `name` 中字段 `key` 对应的 Array 中，将 `data` 合并到包含 `predicate` 的 object 中。返回 boolean 。
...
(function main() {
  ...
  // 检查数据库中 user 字段下是否存在 id 值为 100 的 object
  if (db.includes(name, "user", { id: 100 })) {
    // 获取数据库中 user 字段下 id 值为 100 的 object
    console.log("Object with id is 100 in user:", db.get(name, "user", { id: 100 }));
  }

  // 检查数据库中 data 字段下路径 p1 的数组中是否存在 p2 的值为 "text" 的 object
  if (db.includes(name, "data", "p1", { p2: "text" })) {
    // 获取数据库中 data 字段下路径 p1 的数组中 p2 的值为 "text" 的 object
  ...
  // 删除数据库 user 字段的数组中 id 为 101 的 object
  db.remove(name, "user", { id: 101 });
  // 删除数据库 data 字段下路径 p1 的数组中 p2 为 "text" 的 object
  db.remove(name, "data", "p1", { p2: "text" });
  // 更新数据库 user 字段的数组中 id 为 100 的 object
  ...
  console.log(`Update data for database ${name} ...`);
  // 删除数据库 data 字段下路径 p1[0] 的 object ——如果其值的 object 包含 p22 为 "text2" 的属性
  db.remove(name, "data", "p1[0]", { p22: "text2" });
  // 删除数据库 data 字段下路径 a.b 的 object ——如果其值的 object 包含 c 为 "text" 的属性
