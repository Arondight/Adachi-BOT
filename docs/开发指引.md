...
本项目使用 [Vue.js](https://github.com/vuejs/vue.git) 和 [Puppeteer](https://github.com/puppeteer/puppeteer.git) 协作完成前端部分的工作。如果你的插件需要绘图，那么请在 `../src/views/` 下编写前端部分，并在插件中使用 `../src/utils/render.js` 中的 `render` 方法完成绘图并发送给用户。
