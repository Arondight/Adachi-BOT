...
得到数据库 `name` 的磁盘文件路径。返回 string 。避免以任何方式操作此文件。
...
数据库的数据由 `db.init` 一次性从磁盘读入内存缓存。缓存周期性和在程序退出时自动同步到磁盘，所以尽量避免主动同步数据库。

> 当前数据库缓存的同步周期为五分钟。

##### 同步

将数据库 `name` 的缓存同步到磁盘。返回 boolean 。
...
所有的增删改查只操作缓存，均不涉及磁盘操作。

##### 键的存在性

判断数据库 `name` 中字段 `key` 中是否存在路径 `path` 。返回 boolean 。
...
判断数据库 `name` 中字段 `key` 对应的 Array 中，是否有包含 `predicate` 的对象；
或，数据库 `name` 中字段 `key` 路径 `path` 对应的 Array 中，是否有包含 `predicate` 的对象。返回 boolean 。
...
获取数据库 `name` 中字段 `key` 的对象；
或，数据库 `name` 中字段 `key` 路径 `path` 对应的对象；
或，数据库 `name` 中字段 `key` 对应的 Array 中包含 `predicate` 的对象；
或，数据库 `name` 中字段 `key` 路径 `path` 对应的 Array 中包含 `predicate` 的对象。返回 object 或 `undefined` 。
...
> | `predicate`            | 返回数据库 `name` 中的数据                              |
> | ---------------------- | ------------------------------------------------------- |
> | `undefined` （默认值） | `path` 对应的 Array                                     |
> | object                 | `path` 对应的 Array 中的对象中，包含 `predicate` 的对象 |

##### 设置数据

数据库 `name` 中字段 `key` 中路径 `path` 的数据设置为 `data` 。返回 boolean 。
...
删除数据库 `name` 中字段 `key` 路径 `path` 对应的 Array 中包含 `predicate` 的对象。返回 boolean 。
...
数据库 `name` 中字段 `key` 路径 `path` 对应的 Array 中插入 `data` 。返回 boolean 。
...
(function main() {
  ...
  // 检查数据库中 data 字段下路径是否存在路径 p1 的数组中 p2 的值为 "text" 的对象
  ...
  // 删除数据库 user 字段下 id 为 101 的对象
  db.remove(name, "user", { id: 101 });
  // 删除数据库 data 字段下路径 p1[0] 中 p2 为 "text" 的对象
  db.remove(name, "data", "p1[0]", { p2: "text" });
