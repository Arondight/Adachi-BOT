...
数据库的数据由 `db.init` 一次性从磁盘读入内存缓存。缓存周期性和在程序退出时自动同步到磁盘，所以尽量避免主动同步数据库。当前数据库缓存的同步周期为五分钟。
...
1. 字段 `key` 对应的 Array 中，是否有包含 `predicate` 的对象。
2. 字段 `key` 路径 `path` 对应的 Array 中，是否有包含 `predicate` 的对象。
