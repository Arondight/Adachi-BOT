...
插件的参数 `msg` 接收的 [oicq](https://github.com/takayama-lily/oicq.git) 的数据结构改变。原本的 [CommonMessageEventData](https://github.com/takayama-lily/oicq/blob/39704008d8f00056d435ab8fa96c50f0b88c0568/index.d.ts#L453-L462) 中的 `message` 字段是一个可能包含[这些类型](https://github.com/takayama-lily/oicq/blob/39704008d8f00056d435ab8fa96c50f0b88c0568/index.d.ts#L178-L375)的数组，但是为了统一 `message` 和 `raw_message` 字段，在 `../src/utils/load.js` 中剔除了 [TextElem](https://github.com/takayama-lily/oicq/blob/39704008d8f00056d435ab8fa96c50f0b88c0568/index.d.ts#L188-L194) 之外的所有类型并只保留一个 `TextElem` 。除此之外，将 `raw_message` 和 [GroupMessageEventData](https://github.com/takayama-lily/oicq/blob/39704008d8f00056d435ab8fa96c50f0b88c0568/index.d.ts#L469-L479) 中仅存的 `TextElem` 进行了统一。最后 `GroupMessageEventData` 中新增了字段 `atMe: boolean` 来表示这条消息是否 `@` 了机器人。
...
本项目使用 [Vue.js](https://github.com/vuejs/vue.git) 2 和 [Puppeteer](https://github.com/puppeteer/puppeteer.git) 协作完成前端部分的工作。如果你的插件需要绘图，那么请在 `../src/views/` 下编写网页文件，并在插件中使用 `../src/utils/render.js` 中的 `render` 函数完成绘图并发送给用户。
...
> 1. 你可以使用选项 `viewDebug: 1` 来进行实时前端调试。
> 2. 你也可以在机器人运行时使用命令 `npm run view -- -h` 来查看调试 `../data/record/` 下的数据文件的方法。
