...
判断数据库 `name` 中字段 `key` 对应的 Array 中的对象中，是否有值为 `value` 的路径 `path` 。返回 boolean 。

```js
db.includes(name, key, path, value);
```

##### 获取数据

获取数据库 `name` 中字段 `key` 路径 `path` 对应的 Array 或其中包含 `predicate` 的对象。返回 Object 或者 `undefined` 。
...
(function main() {
  ...
  // 检查数据库中 data 字段下路径是否存在路径 p1[0].p2 的值为 "text" 的对象
  if (db.includes(name, "data", "p1[0].p2", "text")) {
    // 获取数据库中 data 字段下路径 p1[0].p2 的值为 "text" 的对象
    console.log("Object with p2 is text in data.p1[0]:", db.get(name, "data", "p1[0]", { p2: "text" }));
  }
