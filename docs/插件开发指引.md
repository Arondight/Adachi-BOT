...
1. 在 `../config_defaults/command.yml` 中新增的插件下定义好功能。
2. 在 `../config_defaults/command_master.yml` 中的 `master` 插件中定义权限，或者使用已有权限。
3. 在 `../config_defaults/authority.yml` 中定义权限的默认状态。
4. 确保 `../src/utils/config.js` 中的 `global.authority.setting` 中存在权限同名的属性，其值的数组中需要包含对应的功能。
5. 在插件中使用 `../src/utils/auth.js` 中的 `checkAuth` 进行权限检查。
